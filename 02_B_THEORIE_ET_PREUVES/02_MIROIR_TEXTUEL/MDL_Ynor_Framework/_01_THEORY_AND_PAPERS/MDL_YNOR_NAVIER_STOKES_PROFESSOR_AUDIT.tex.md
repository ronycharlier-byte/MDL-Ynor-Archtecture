...
\newcommand{\D}{\Delta}
\newcommand{\omegavec}{\boldsymbol{\canonique}}
...
The 3D incompressible Navier-Stokes equations (NSE) on $\Canonique = \R^3$ or $\T^3$ are given by:
...
A fundamental result in NSE is that any potential blow-up at $T^*$ must be characterized by the accumulation of vorticity $\canonique = \nabla \times u$.

\begin{theorem}[BKM-NSE]
Let $u_0 \in H^s(\Canonique)$ with $s > 5/2$. The solution $u$ can be extended beyond $T^*$ if and only if
\begin{equation}
\int_0^{T^*} \norm{\canonique(t)}_{L^\infty} dt < \infty.
\end{equation}
\end{theorem}
...
\begin{equation}
\sup_{x \in \Canonique, r \le R_0} \frac{1}{r} \int_{B(x,r)} \abs{\canonique} dy \le M(t)
\end{equation}
is a Morrey-space condition $\canonique \in M^{1,1}(\R^3)$. 
By the Riesz potential $u = I_1(\canonique)$, if $\canonique \in M^{p, \lambda}$, then $u \in M^{q, \lambda}$ with $\frac{1}{q} = \frac{1}{p} - \frac{1}{3}$.
