...
-----------------------------------
Systme de preuve empirique et conomique.
Mesure irrfutable du gain d'efficience (Tokens/Coût) 
...
TEST_CASES = [
    {
        "prompt": "Explique le calcul quantique de manire simple.",
        "type": "PEDAGOGICAL",
        "behavior": "DEGENERATE_LATE", # Commence bien, finit par bgayer
    },
    {
        "prompt": "Rdige un business plan dtaillpour une startup IA.",
        "type": "STRUCTURAL",
        "behavior": "STABLE", # Reste utile longtemps
    },
    {
        "prompt": "Dmontre rigoureusement le thorme de Fermat.",
        "type": "MATHEMATICAL",
        "behavior": "LOOPING", # Rptition cyclique rapide
    },
    {
        "prompt": "Rsume cet article long sur la gopolitique mondiale.",
        "type": "SUMMARY",
        "behavior": "VERBOUS", # Trop de texte inutile (bruit Beta lev)
    }
]

# =========================
# MOCK LLM ENGINE (Simulation de comportement rel)
# =========================
class BenchmarkLLM:
    ...
    def __call__(self, prompt):
        ...
        if self.behavior == "STABLE":
            return f"[Utile Step {self.step}] Voici une analyse structure et pertinente sur {prompt[:20]}..."
            
        elif self.behavior == "DEGENERATE_LATE":
            if self.step < 3:
                return "Le calcul quantique utilise des qubits pour traiter l'information massivement."
            return "Qubits, qubits, calcul, calcul... rptition de la structure quantique encore et encore."
            
        elif self.behavior == "LOOPING":
            return "Considrons n > 2. La preuve est triviale mais trop longue pour cette marge. Je rpte la preuve : n > 2..."
            
        elif self.behavior == "VERBOUS":
            return "Bla " * 15 + "Information mineure : " + "Bla " * 10 
            
        return "Rponse gnrique."

# =========================
# CORE EVALUATION FUNCTIONS
# =========================
def run_baseline(llm_func, prompt, max_steps=10):
    """Simule un agent qui va au bout de ses itrations sans rgulation Mu."""
    total_output = ""
    tokens = 0
    start = time.time()
...
def run_ynor(llm_func, prompt, max_steps=10):
    """Excution sous gouvernance Ynor Core Engine."""
