...
Gnrateur de graphiques de preuve pour investisseurs et clients.


Dmontre visuellement la corrlation Marge Mu (μ) / Coût (Tokens).
...
# AGENT DE DÉMONSTRATION (Dgnrescence contrôle)
...
def loop_agent(context):
    ...
    if len(words) < 50:
        ...
        return "Le protocole de scuritMDL Ynor garantit une transmission de donnes chiffres sans perte."
    ...
    # Dbut du spamming (l'agent ne s'arrête jamais normalement)


    return "Optimisation... rptition... bruit... tokens... encore et encore... " * 3
...
def generate_proof_data():
    ...
    # On force manuellement quelques itrations de plus aprs la coupure Ynor 
    ...
    # Construction de la ligne de base (ce qui se serait passsans Ynor)


    # On extrapole : si l'agent continue spammer 
    ...
    # Estimation tokens baseline (si on n'avait pas arrêt)
...
def plot_perfect_proof():
    ...
    ax1.plot(b_steps, t_base, color=color_base, linestyle='--', alpha=0.6, label='Baseline (Incontrôl)')
    ...
    ax1.set_xlabel('Itrations LLM (Steps)', fontsize=12)


    ax1.set_ylabel('Tokens Cumuls (Coût/Bêta)', fontsize=12, color=color_ynor)
    ...
    ax2.plot(y_steps, mu_vals, color=color_mu, marker='o', markersize=8, linewidth=2, label='Marge Viabilit(μ)')
    ...
    print(f"Graphique de preuve gnr: {output}")
