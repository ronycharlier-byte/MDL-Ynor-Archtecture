...
def find_identical_files():
    ...
    print(f"[1/2] Scan de {total_scanned} fichiers terminé.")
    print(f"[2/2] {len(duplicates)} doublons exacts identifiés.")
...
if __name__ == "__main__":
    ...
    ts = 689 # Valeur obtenue au scan précédent
    dups = report_data['duplicates_found']
    print(f"\n[RÉSULTAT] Gain Potentiel : Supprimer {dups} fichiers.")
    print(f"[RÉSULTAT] Nouveau Taux Collapsé Estimé : {((dups / ts) * 100):.2f}% de Gain Immédiat.")
