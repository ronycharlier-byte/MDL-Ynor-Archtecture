﻿"""
runway_renumber.py
But: calculer le numro magntique d'une piste et indiquer si renumrotation ncessaire.
...
def get_declination_noaa(lat, lon, date_str="2026-01-01", result_format="json"):
    """
    Appelle l'API NOAA pour obtenir la dclinaison (en degrs).
...
def true_to_magnetic_heading(true_heading_deg, declination_deg):
    ...
    declination_deg : angle (degrs), positif = champ magntique l'est de vrai nord
...
def runway_number_from_magnetic(magnetic_heading_deg):
    ...
    Rgle FAA: arrondir au 10 degrs le plus proche, diviser par 10, 0 -> 36
    """
...
def check_airport_renumbering(airport_name, lat, lon, true_heading):
    """
    Compare le numro drivavec la dclinaison 2015 et 2026.
    """
    print(f"\n--- Audit Magntique : {airport_name} ---")
    ...
    print(f"Dclinaison 2015: {decl_2015:+.2f}° -> Numro: {num_2015:02d}")
    print(f"Dclinaison 2026: {decl_2026:+.2f}° -> Numro: {num_2026:02d}")
...
if __name__ == "__main__":
    # Test Austin, Texas (Exemple de la vido)
    check_airport_renumbering("Austin Bergstrom (AUS)", 30.1944, -97.67, 184.1)
    
    # Test Fairbanks, Alaska (Exemple de la vido)
    check_airport_renumbering("Fairbanks Int (FAI)", 64.815, -147.856, 19.3)
    
    # Test Genve, Suisse (Exemple de la vido)
    check_airport_renumbering("Genve (GVA)", 46.2381, 6.1089, 44.5)
