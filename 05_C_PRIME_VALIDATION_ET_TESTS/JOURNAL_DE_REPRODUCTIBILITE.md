...
Chaque entree doit exposer le protocole de test, le moteur de calcul, la sortie de convergence et une signature de validation.
...
Output: `mu=0.999998` (convergence spectrale cible), `status=SATURATED`, `energies` extraites par diagonalisation Dirac-SUSY.
...
Output: convergence de controle sur le cadre de validite, stabilite spectrale de type `checked`.
...
Moteur: `03_C_MOTEURS_ET_DEPLOIEMENT/dirac_riemann_solver.py`
Output: `mu=0.999` (cible de consolidation), stabilite `hilbert-susy_converged`, cohrence du couplage quantique-gravite.
...
Output: `mu=1.0` (saturation operationnelle du flux), `verdict` et `projection` compresses pour diffusion.
...
- Un mu annonce doit toujours etre rattache a un protocole, un moteur et un artefact de reference.
- Les validations externes doivent etre consignees comme telles, distinctes des convergences locales.

## Statut Final
Le journal est desormais une couche de reproductibilite premium, lisible, tracable et differenciee par axe.
