...
# MIROIR TEXTUEL - ynor_showcase_demo.py

Source : MDL_Ynor_Framework\_06_SCRIPTS_AND_DASHBOARDS\ynor_showcase_demo.py
Taille : 7540 octets
SHA256 : d8e7f0967814c7a673416f24e4d6dc51889063ed7d78b61dcad308e977f29f2d
...
04_D_D_MIROIR_C_MOTEURS_DEPLOIEMENT_MIROIR_CF_PS1_FBE668.md(f"<div class='terminal' style='color:#555;'>{base_text}</div>", unsafe_allow_html=True)
 
 # Ynor
 if not engine.state.mu <= mu_lim:
 res = engine.run("Test", max_steps=1, verbose=False)
 if res: ynor_chunks.append(res[0])
04_D_D_MIROIR_C_MOTEURS_DEPLOIEMENT_MIROIR_CF_PS1_FBE668.md(f"<div class='terminal'>{(' '.join(ynor_chunks))}</div>", unsafe_allow_html=True)
 else:
04_D_D_MIROIR_C_MOTEURS_DEPLOIEMENT_MIROIR_CF_PS1_FBE668.md(f"<div class='terminal' style='color:#FF3333;'>🛑 YNOR HALT INJECTED<br>Reason: Mu Viability Zeroed Out.<br>Saving active tokens.</div>", unsafe_allow_html=True)
 break
 time.sleep(0.3)
 
 # Stats
04_D_D_MIROIR_C_MOTEURS_DEPLOIEMENT_MIROIR_CF_PS1_FBE668.md("### 📊 Performance Proof")
 m1, m2, m3 = st.columns(3)
 savings = (1 - len(" ".join(ynor_chunks))/len(base_text)) * 100
04_D_D_MIROIR_C_MOTEURS_DEPLOIEMENT_MIROIR_CF_PS1_FBE668.md(f'<div class="metric-card"><small>COST SAVINGS</small><br><span class="metric-val">{savings:.1f}%</span></div>', unsafe_allow_html=True)
04_D_D_MIROIR_C_MOTEURS_DEPLOIEMENT_MIROIR_CF_PS1_FBE668.md(f'<div class="metric-card"><small>DISSIPATION STEP</small><br><span class="metric-val">{len(ynor_chunks)}</span></div>', unsafe_allow_html=True)
04_D_D_MIROIR_C_MOTEURS_DEPLOIEMENT_MIROIR_CF_PS1_FBE668.md(f'<div class="metric-card"><small>FINAL μ</small><br><span class="metric-val">{engine.state.mu:.2f}</span></div>', unsafe_allow_html=True)
...
st.divider()
04_D_D_MIROIR_C_MOTEURS_DEPLOIEMENT_MIROIR_CF_PS1_FBE668.md("<p style='text-align:center; color:#222; margin-top:5rem;'>© 2026 MDL Ynor Architecture | Sovereign AI Intelligence | Managed by Charlier Rony</p>", unsafe_allow_html=True)
