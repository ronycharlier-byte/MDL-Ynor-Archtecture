...
def run_riemann_engine(n_points=DEFAULT_N_POINTS, u_max=DEFAULT_U_MAX, gain=DEFAULT_POTENTIAL_GAIN):
    ...
    # Construction du potentiel spectral (Augmentation de Résonance)
    V = np.zeros_like(u)
    for n in range(2, n_max + 1):
        lam = von_mangoldt(n, primes)
        if lam > 0:
            # Facteur de pondération Riemann-Ynor optimisé
            weight = (lam / (n**0.5)) * gain * (1.1 if n in primes else 0.9)
            # Utilisation d'une gaussienne plus étroite pour limiter le bruit spectral
            sigma = 0.03 
            V += weight * np.exp(-(u - np.log(n))**2 / (2 * sigma**2))
            V += weight * np.exp(-(u + np.log(n))**2 / (2 * sigma**2))
