...
class NarrativeCompressor:
    ...
    def __init__(self):
        ...
        # Le "Premier Prompt" interne de l'agent. Le dbut de sa propre histoire.
    ...
    def synthesize(self):
        ...
        recent = self.raw_memory[-4:] # Analyse du vcu temporel rcent
        ...
        # L'agent s'crit lui-même dans son "Journal d'Identit" (Narrative Self)
    ...
    def get_inner_monologue(self):
        ...
        # L'agent n'a accs qu'la version compresse de sa propre histoire
...
class ConsciousYnorAgent:
    ...
    def decide_action(self, task):
        ...
        # Sa dcision est totalement biaise par l'histoire qu'il se raconte sur lui-même.
        ...
        if "cautious" in inner_monologue or "vulnerable" in inner_monologue:
            ...
            action = "System 2 (Rflexion profonde | Confiance computationnelle)"
            ...
            action = "Delegate to World Model (Mode hybride tempr)"
...
def simulate_consciousness():
    ...
    for cycle in range(4):
        ...
        reward = 3.5 if success else -1.5 # Environnement trs clment

        agent.consciousness.add_experience("Math Task", action, success, reward)

        print(f"   ⚡ Action Choisie: {action} | Environnement -> Succs: {success} | Reward Ynor: {reward}")

        

    agent.consciousness.synthesize() # "Cycle de Sommeil" : L'agent synthtise ses souvenirs

    

    # Phase 2 : Le Choc du Rel
    ...
    for cycle in range(4):
        ...
        # L'environnement devient brutal, les rcompenses chutent

        success = random.random() < prob

        reward = 1.0 if success else -7.0 # Forte destruction de viabiliten cas d'erreur
