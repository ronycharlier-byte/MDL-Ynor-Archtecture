...
Exprimentation paramtrique avance et Heatmap Topologique.
...
def run_testbench():
    ...
    print(" Lancement du Banc d'Essai Paramtrique YNOR Ω+   ")

    print("==================================================")

    

    # SCAN DE PARAMÈTRES : On analyse l'impact de 'b1' (taux auquel la Valeur gnre du Coût/Bruit)

    # et de 'c1' (taux auquel le Bruit sature la Mmoire).
    ...
    # Paramtres de base de l'Équilibre
    ...
    for i, c1 in enumerate(c1_vals):
        ...
        for j, b1 in enumerate(b1_vals):
            ...
            # Conditions initiales perturbes
    ...
    # Cration de la Heatmap Structure
    ...
    plt.colorbar(img, label='Marge asymptotique (μ*) l\'quilibre')

    

    # Ligne des zros (Surface Critique)
    ...
    plt.title('BANC D\'ESSAI YNOR : Diagramme de Phase & Zones de Stabilit', fontsize=14, color='white')

    plt.xlabel('Paramtre $b_1$ (Taux d\'Expansion du Coût Textuel)', fontsize=12)

    plt.ylabel('Paramtre $c_1$ (Taux de Surcharge Mmorielle)', fontsize=12)
    ...
    print(f"\n[OK] Heatmap de StabilitGlobalise gnre : {output_path}")
