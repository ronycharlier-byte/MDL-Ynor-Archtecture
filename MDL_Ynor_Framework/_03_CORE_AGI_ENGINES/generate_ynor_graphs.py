...
plt.plot(difficulties, ynor_computes, marker='o', color='#2ca02c', linewidth=2.5, markersize=8, label="AGI Ynor (Compute Auto-Rgul)")

plt.axhline(y=15, color='#d62728', linestyle='--', linewidth=2, label="Baseline LLM (Compute Statique)")

plt.title("Fig 1: Scaling Computationnel Endogne vs Difficult", fontsize=14, fontweight='bold')

plt.xlabel("Difficultde la Synthse (Facteur d'attnuation de l'erreur)", fontsize=12)

plt.ylabel("Ressources physiques alloues (Steps)", fontsize=12)

plt.fill_between(difficulties, ynor_computes, 15, color='#d62728', alpha=0.1, label="Gaspillage d'nergie LLM")
...
cbar.set_label("Marge de ViabilitYnor ($\mu$)", fontsize=12)
...
plt.title("Fig 2: La Frontire de Pareto Infrentielle Ynor", fontsize=14, fontweight='bold')
...
plt.plot(steps, L_ynor, color='#1f77b4', linewidth=3.5, label="Action Ynor Complte ($S_{Ynor}$)")

plt.plot(steps, L_no_c, color='#ff7f0e', linestyle='--', linewidth=2, label="Ablation NO_C ($\Rightarrow$ Crash par sur-calcul)")

plt.plot(steps, L_no_d, color='#9467bd', linestyle='-.', linewidth=2, label="Ablation NO_D ($\Rightarrow$ Halt trop prcoce)")

plt.plot(best_idx+1, L_ynor[best_idx], marker='*', color='gold', markersize=20, markeredgecolor='black', label="Attracteur de Survie AGI")

plt.title("Fig 3: Dynamique Variationnelle de la Viabilit", fontsize=14, fontweight='bold')

plt.xlabel("Profondeur de calcul (steps)", fontsize=12)

plt.ylabel("Marge de ViabilitGlobale ($\mu$)", fontsize=12)
...
print("Figures 1, 2 et 3 gnres et sauvegardes avec succs !")
