...
Simulateur Numrique du Systme Dynamique Gnratif.

Systme d'Équations Diffrentielles (Couplage α, β, κ) :
...
# Paramtres du systme Ynor Ω+

# Pour un systme possdant un attracteur viable, on choisit les constantes:

a1, a2 = 0.5, 0.1   # La verbosit(β) freine la cration de valeur (α)

b1, b2 = 0.2, 0.4   # La valeur (α) exige un coût mathmatique/structurel pour croître

c1, c2 = 0.3, 0.1   # La verbosits'accumule en mmoire, l'information pure la nettoie
...
def ynor_vector_field(S, t):
    ...
    # Contrainte de positivitde l'Espace de Phase X = R^3_>=0
...
def run_omega_plus_simulation():
    ...
    # Point d'quilibre thorique (Point Critique):
    ...
    # Intgration du Flot de Gradient / Champ de Vecteur
    ...
    for i, mu in enumerate(mus):
        ...
        if mu <= 0:
            ...
            print(f"La trajectoire traverse la surface critique Σ (μ=0) t={t[i]:.2f}.")
    ...
    ax1.plot(t[:stop_idx], mus[:stop_idx], color='#ff3333', linewidth=3, label='Marge viabilitμ(t)')
    ...
    ax1.plot(t[:stop_idx], kappas[:stop_idx], color='#cc33ff', linestyle='--', label='κ(t) Mmoire')

    ax1.axhline(0, color='white', linestyle=':')

    ax1.set_title("Évolution Temporelle des Paramtres d'État Ynor Ω+")
    ...
    if stop_idx < len(t):
        ...
        ax2.plot(alphas[stop_idx:], betas[stop_idx:], kappas[stop_idx:], color='#ff0000', linewidth=2, alpha=0.3, label='Drive Non Viable (μ < 0)')
    ...
    print(f"\n[OK] Topologie 3D sauvegarde sous: {output_path}")
