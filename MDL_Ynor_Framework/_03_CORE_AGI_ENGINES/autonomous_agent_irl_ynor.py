...
def log_audit_action(action, detail):
    ...
    """Enregistre toute tentative d'criture dans un journal d'audit infalsifiable."""
...
class AutonomousIRLYnorAgent:
    ...
    def get_environment_state(self):
        ...
        # Perception relle simule (ex: observe le dossier, la bourse, l'heure)
    ...
    def evaluate(self, result, cost):
        ...
        # Fonction de rcompense (Ynor proxy : max info / min coût)

        base_reward = 10.0 if "success" in result.lower() or "result" in result.lower() else 1.0

        # Sanction sur le coût pour forcer l'optimisation mathmatique

        return base_reward - (0.5 * cost)

    def self_improve(self):

        # L'agent analyse ses rcompenses rcentes et met jour ses poids (Mock)
    ...
    def run_infinite_loop(self, max_iter=5):
        ...
        while True:
            ...
            print(f"📈 Rsultat: {result[:50]}... | Reward: {reward:.2f} | Coût total: {self.total_cost:.1f}")
            ...
            if iteration >= max_iter:
                ...
                print("\n>>> FIN DE LA SIMULATION (ScuritAnti-Runaway active pour le test)")
