...
def plot_mu_heatmap(a2_values, b1_values, mu_grid):
    ...
    plt.xlabel("Paramtre a2 (Inhibition de Valeur par le Bruit)")

    plt.ylabel("Paramtre b1 (Cration de Bruit par la Valeur)")

    plt.title("Heatmap Ynor : Espace Paramtrique de Marge Dissipative")
    ...
    print(f"[OK] Heatmap MU sauvegarde : {output}")
...
def plot_regime_map(a2_values, b1_values, regime_grid):
    ...
    plt.xlabel("Paramtre a2")

    plt.ylabel("Paramtre b1")

    plt.title("Classification Topologique des Rgimes (Stables vs Crash)")
    ...
    print(f"[OK] Heatmap de Classification sauvegarde : {output}")
...
if __name__ == "__main__":
    ...
    print("Construction de la Cartographie 2D des Rgimes...")

    a2_values = np.linspace(0.1, 2.0, 80)

    b1_values = np.linspace(0.1, 2.0, 80)

    # Rduire 'dt' lgrement pour la rapiditdu calcul sur la grille de 80x80 (6400 points)
