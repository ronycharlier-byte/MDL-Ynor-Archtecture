...
Hautement Reproductible Landing Page + Live Engine Demo.
...
# CUSTOM CSS (HIGHLY REPRODUCIBLE DARK MODE)

st.markdown("""

    <style>

    @import url('https://fonts.googleapis.com/css2family=JetBrains+Mono&family=Inter:wght@400;900&display=swap');
...
with c1:
    ...
    scenario = st.selectbox("Failure Scenario", ["Dgnrescence Rptitive", "Boucle Infinie", "Bruit Verbeux (Hallucination)"])
...
if test_btn:
    ...
    def mock_logic(c): 
        ...
        if scenario == "Dgnrescence Rptitive": return "REPETITION " * 5 + f" of node {len(c)}"
