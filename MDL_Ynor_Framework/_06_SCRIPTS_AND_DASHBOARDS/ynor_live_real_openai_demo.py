...
Dmonstration en temps-rel utilisant l'API OpenAI (GPT-4o-mini).

Compare une excution standard brute face la gouvernance Ynor.
...
# SDK Integration (pour les comptages de tokens prcis)
...
class YnorState:
    ...
    def mu(self):
        ...
        # Marge dissipative simple pour la dmo

        return self.alpha - self.beta - self.kappa

def measure_alpha(output):

    # Proxy : Densitd'information unique
...
with st.sidebar:
    ...
    st.info("Cette dmo compare une excution standard une excution protge par Ynor.")
...
st.write("Dmontrez l'arrêt intelligent et l'conomie de coûts sur de vrais prompts.")

user_prompt = st.text_area("Entrez un prompt complexe (ex: raisonnement itratif, rdaction longue)", 

                         "Effectue une analyse itrative dtaille de l'impact de l'IA sur l'conomie mondiale en 10 tapes.")

if st.button("Lancer la Dmo Comparative"):

    if not api_key:

        st.error("Veuillez fournir une clAPI OpenAI dans la barre latrale.")
        ...
        with st.status("Excution Baseline (Standart)..."):
        ...
        with st.status("Excution YNOR Core (Live Audit)..."):
            ...
            # Simulation multi-tapes ou gnration contrôle

            # Pour la dmo, on simule une boucle de rflexion / gnration atomique
            ...
            for step in range(8):
                ...
                b = get_token_count(chunk) / 50 # Coût normalis
                ...
                if state.mu <= mu_threshold:
                    ...
                    st.warning(f"🛑 YNOR STOP : Marge mu puise l'tape {step+1} (mu={state.mu:.2f})")
        ...
        if mu_trace:
            ...
            st.caption("La gnration s'arrête ds que la courbe rouge descend sous le seuil dfini.")
