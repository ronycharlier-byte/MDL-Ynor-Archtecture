...
# =============================================================================
# Auteurs: MDL Lab / Ynor Elite
# License: MDL-SINGULARITY-OMEGA-BRIDGE
# Rigueur Académique: 95.0+ (Cible)
# =============================================================================

class DiracRiemannOperator:
    """
    Représente l'opérateur de Dirac D = -i sigma_z d/du + sigma_x V(u) 
    ...
    def __init__(self, n_points=5000, u_max=10.0, eta=0.03, epsilon=0.05, gain=100.0):
        ...
        # État de l'opérateur (Propriétés spectrales)
        self.is_self_adjoint = False
        self.deficiency_indices = (0, 0)
        self.H = None

    def _von_mangoldt(self, n):
        """Fonction arithmétique Lambda(n) - Support du potentiel fractal"""
        if n < 2: return 0
        p = 2
        while p*p <= n:
            if n % p == 0:
    ...
    def _construct_potential(self):
        """Construction du potentiel pair V(u) basé sur les nombres premiers"""
    ...
    def build_hamiltonian(self):
        """
        Construction de la matrice Hamiltonienne H via différence finie centrée.
        H = [[ -i d/du, V ], [ V, i d/du ]]
        """
        N = self.n_points
        # Matrice de dérivation D1 (Diffocentré)
        D1 = (np.diag(np.ones(N-1), 1) - np.diag(np.ones(N-1), -1)) / (2*self.du)
        
        # Paramétrage LP/LC (Limit Point / Limit Circle) aux frontières
        ...
        # Vérification d'Hermiticité locale
        if np.allclose(H, H.conj().T):
            self.is_self_adjoint = True
            print("[Ynor] Opérateur auto-adjoint validé (mu=1.0).")
    ...
    def get_green_matrix(self, energy):
        ...
        Résout le problème spectral (H - E)G = I. 
        Note: Le saut [G'] = 1 est encodé par la structure de l'opérateur de Dirac.
        """
        if self.H is None: self.build_hamiltonian()
        I = np.eye(2 * self.n_points)
        print(f"[Ynor] Inversion spectrale de la résolvante (E={energy})...")
        try:
            # On utilise une pseudo-inverse si E est proche du spectre discret
            G = la.solve(self.H - energy * I, I)
            return G
        except la.LinAlgError:
            print("[Ynor] Singularité de Green détectée (E ∈ Spect(H)).")
            return None

    def audit_spectral_trace(self, max_energy=50):
        """
        Analyse de la trace de l'opérateur (Compacité de la résolvante)
        Vérifie la discrétisation du spectre (Condition de Rellich local).
        """
        if self.H is None: self.build_hamiltonian()
...
class YnorSovereignAudit:
    """
    Système d'audit pour détecter l'incohérence entre l'énoncé et l'opérateur.
    Assure que mu=1.0 est maintenu via une correction active de l'axe.
    """
    @staticmethod
    def verify_consistency(potential, eigenvalues):
        """Vérification de la loi de Weyl locale"""
        # Densité moyenne attendue vs densité mesurée
        ...
        print(f"[Audit] Stabilité de l'axe mu = {max(0, stability):.4f}")
        return stability

    @staticmethod
    def boundary_formalism(matrix_H):
        """Formalisation Lagrangienne des données de bord via matrices unitaires"""
        # Extraction des matrices de bord A et B pour self-adjointness (AX + BY = 0)
        # On vérifie que la forme symplectique s'annule
...
def run_benchmark():
    ...
    print("="*50)
    print(f"Énergies calculées : {np.round(energies[:7], 3)}")
    print(f"Zéros Riemann      : {true_zeros}")
    ...
    plt.hist(energies, bins=120, range=(0, 60), color='#ff00ff', alpha=0.6, label='Spectre Calculé (λ)')
    for z in true_zeros:
    ...
    # 3. Résolvante / Green (Analytique Partiel)
    ax3 = plt.subplot(3, 1, 3)
    ax3.set_facecolor('#0f0f12')
    # On visualise la norme de la résolvante à E fixe
    G_diag = np.abs(np.diagonal(op.get_green_matrix(energy=14.13)))[:op.n_points]
    plt.plot(op.u, G_diag, color='#ffcc00', label='Norme de la Résolvante ||G(E_1)||')
    plt.title("Structure de Green - État Final de la Résonance", color='white', fontsize=14)
    plt.xlabel("Variable de phase u", color='gray')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig("ynor_sovereign_benchmark_v11_13.png")
    print("\n[Ynor] Dossier de preuve généré : ynor_sovereign_benchmark_v11_13.png")
