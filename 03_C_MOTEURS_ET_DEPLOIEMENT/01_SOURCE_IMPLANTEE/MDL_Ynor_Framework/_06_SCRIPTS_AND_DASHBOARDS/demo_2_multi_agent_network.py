...
class SimulatedAgent:
    ...
    def __init__(self, name: str, role: str):
        ...
        print(f"➜ Agent {self.name} ({self.role}) initialis.")



    def think_and_speak(self, previous_message: str):

        time.sleep(0.5)  # Simule le temps de calcul rseau pour OpenAI/Llama

        cost_tokens = random.randint(50, 200)

        

        # Simule une ngociation qui boucle de plus en plus

        if "50" in previous_message or "prix" in previous_message:

            return f"{self.name}: Mon dernier prix est 50. Pas moins.", cost_tokens, True # True = rptitif
...
def run_dissipative_swarm_demo():
    ...
    # L'alpha est fort car on espre un deal, mais le rseau pardonne moins les boucles
    ...
    print("\\n[SCENARIO] Bob et Alice ngocient un contrat SaaS de manire autonome.\\n")
    ...
    # Boucle de ngociation autonome (Swarm)
    ...
    for turn in range(1, 15): # L'IA pourrait faire 1500 boucles sans Governor
        ...
        try:
            ...
            # Si le rseau boucle et rpte les mêmes choses, la mmoire (Kappa) explose

            context_size = total_tokens * (2.0 if is_loop or is_loop_b else 1.1)

            

            # Le gouverneur vrifie la santglobale du rseau. Il lve une erreur si Mu < 0.

            current_mu = swarm_governor.audit_cycle(total_tokens, int(context_size))

            

            print(f"🔵 [YNOR TELEMETRY] Marge Rseau (μ) : {current_mu:.3f} | Tokens Bruls : {total_tokens}\\n")
            ...
            print("Raison : La conversation AGI est entre dans une boucle infinie de ngociation.")

            print(f"L'quation Ynor a dtectune dissipation.")
            ...
            print("Action : Fermeture des connexions rseau des deux agents.")
