...
with search_col:
    ...
    if search_query:
        try:
            results = _api_get(
                api_base,
                f"/api/corpus/searchq={quote(search_query)}&limit={search_limit}",
            )
...
try:
    files_payload = _api_get(api_base, f"/api/corpus/fileslimit={top_limit}&offset=0")
...
try:
    full_listing = _api_get(api_base, "/api/corpus/fileslimit=2000&offset=0")
