...
class CulturalMemory:
    ...
    def get_best_norm(self, context):
        ...
        # Le "marchdes ides" : slectionne la norme la plus forte pour le contexte
    ...
    def cull_dead_norms(self):
        ...
        # Mort par slection naturelle culturelle

        dead = [n for n in self.norms if n.score <= 0.0]

        for d in dead:

            print(f"  💀 [CULTURAL DEATH] La norme '{d.action} for {d.condition}' a tlimine par la matrice de survie (Score <= 0).")
...
class EvolutionaryCultureSimulator:
    ...
    def run_epoch(self, cycles=50):
        ...
        for cycle in range(1, cycles + 1):
            ...
            # 1. Slection culturelle (Darwinisme)

            applied_norm = self.memory.get_best_norm(task_context)

            if not applied_norm:

                continue # Culture effondre sur ce contexte
