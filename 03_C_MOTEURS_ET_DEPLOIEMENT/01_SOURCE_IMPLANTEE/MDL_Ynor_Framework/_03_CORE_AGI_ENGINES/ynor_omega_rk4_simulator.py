...
def simulate(T=40, dt=0.01, init=(0.5, 1.5, 1.0)):
    ...
    for i in range(N-1):
        ...
        # Application de la PositivitStricte (A1)
...
def plot_results(t, S, mu):
    ...
    plt.plot(t, S[:,2], label="kappa (Mmoire)", color='#cc33ff')
    ...
    plt.title("Dynamique Ynor Ω+ (Intgration RK4)")
    ...
    print(f"Graphique dynamique sauvegard: {output}")
...
def run_expert_scan():
    ...
    plt.xlabel("Paramtre a2 (Sensibilitde Valeur au Coût)")


    plt.ylabel("Marge μ finale l'quilibre")


    plt.title("Scan Ynor Ω+ : Zone de stabilit(Bifurcation selon a2)")
    ...
    print(f"Graphique de Bifurcation sauvegard: {output}")
...
if __name__ == '__main__':
    ...
    print(f"Stabilitasymptotique garantie : {results['stable']}")


    print(f"Temps de recouvrement critique t*: {results['t_recovery']}")


    print(f"Marge Rsiduelle Émergente μ* : {results['final_mu']:.4f}")
