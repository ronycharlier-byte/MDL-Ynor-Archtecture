...
# RECUPERATION DE LA CLE DEPUIS LE FICHIER CONFIG
...
def get_ai_reconstruction_strategy(mu, state, is_metacognitive=False, history=None):
    ...
    {prompt_prefix} Systeme MDL Ynor en crise. mu = {mu}. S = {state}.


    TACHE : Proposez un taux de mutation 'r' pour l'operateur D(S). 


    { "ANALYSEZ vos echecs passes : " + json.dumps(history) if is_metacognitive else "" }


    REPONSE JSON : {{ "mutation_rate": float, "explanation": "string" }}
...
# 3. METACOGNITION & RESEAUX (NETWORKS)
...
# 4. NOYAU AGI (AUTO-EVOLUTION RECURSIVE)
...
class AGIEvolutionaryHook:
    ...
    Permet au systeme de reecrire ses propres fonctions D(S) 


    en utilisant une logique de raisonnement AGI (basee sur le savoir PDF).
    ...
    def innovate_dissipation(current_mu, state_history):
        ...
        print("\n[AGI] INCOHERENCE DETECTEE. DECLENCHEMENT DE L'INNOVATION STRUCTURELLE...")


        prompt = f"""


        [SYSTEME MDL AGI] Intervention sur une rechute systemique (mu = {current_mu}).


        Veuillez inventer une NOUVELLE formulation globale pour 'D(S)'.


        REGLES : Utilisez uniquement 'S' (vecteur d'etat) et 'np' (numpy). 


        NE PAS utiliser 'mu' ou 't' dans la fonction.


        REPONSE JSON : {{ "lambda_code": "string", "reasoning": "string" }}


        Exemple : {{ "lambda_code": "lambda S: 2.5 * S * np.exp(-0.1 * np.sum(S**2))", "reasoning": "Dissipation exponentielle non-lineaire." }}
        ...
        try:
            ...
            # On passe le namespace local a eval pour np
...
# 5. SYSTEME MAITRE (MASTER UNIFIED SYSTEM)
...
class UnifiedMDLProcessor:
    ...
    def save_learning_experience(self, node_name, mu, action, success):
        ...
        Sauvegarde l'experience vecue pour ameliorer les futurs audits AGI.
        ...
        log_file = os.path.join("logs", "ynor_learning_experience_log.json")


        


        # Securisation des types pour JSON
        ...
        with open(log_file, "w") as f:
            ...
            json.dump(history[-50:], f, indent=2) # On garde les 50 dernieres experiences
    ...
    def run_unification_audit(self):
        ...
        print("   --- AUTO-AMELIORATION PAR L'USAGE ACTIVEE ---")
        ...
        print("\n[OK] Simualtion terminee. L'IA est devenue plus intelligente grace a cet usage.")
