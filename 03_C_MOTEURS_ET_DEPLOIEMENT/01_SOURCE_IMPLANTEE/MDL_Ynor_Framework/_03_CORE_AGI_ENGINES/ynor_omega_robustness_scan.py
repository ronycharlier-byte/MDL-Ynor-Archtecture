...
def robustness_score(base_params, n_trials=15, T=40, dt=0.05, init=(0.5, 1.5, 1.0), eps=0.05):
    ...
    for _ in range(n_trials):
        ...
        # Test de viabilitabsolue (au-delde la marge d'erreur)
...
def build_robustness_map(a2_values, b1_values):
    ...
    for i, b1 in enumerate(b1_values):
        ...
        for j, a2 in enumerate(a2_values):
            ...
            # 15 mont-carlo par point tensoriel (dt=0.05) pour quilibre perf/precision
            ...
            if current % 400 == 0:
                ...
                print(f"Progression : {current}/{total_iters} blocs matriciels calculs...")
...
def plot_robustness(a2_values, b1_values, grid):
    ...
    # La colormap 'magma' met en valeur les zones teintes (noir) et vibrantes (jaune/blanc)
    ...
    plt.colorbar(im, label="Indice de Robustesse Ynor (0: Implosion → 1: StabilitInfinie)")


    plt.xlabel("Paramtre a2 (Sensibilitau Bruit)")


    plt.ylabel("Paramtre b1 (Densitdu Bruit Gnr)")


    plt.title("Carte de Robustesse Mmorielle sous Stress Stochastique")
    ...
    print(f"\n[OK] Heatmap de Robustesse sauvegarde physiquement : {output}")
...
if __name__ == '__main__':
    ...
    print(" Modle de perturbation Monte-Carlo sur drives LLM alatoires...")
    ...
    # Grille de rsolution 40x40 (1600 pixels) * 15 essais = 24 000 simulations compltes RK4
