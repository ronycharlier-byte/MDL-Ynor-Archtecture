...
def inference_error(diff_factor, step, noise_level):
    ...
    # La divergence baisse avec l'itration mais avec un plancher stochastique
...
def run_trial(diff_factor, noise_level, max_steps, mode="ynor", alpha=0.2, beta=0.05):
    ...
    # 1. Gnration de la trajectoire (même environnement pour tous les modes)
    ...
    for s in range(1, max_steps + 1):
        ...
        # La vraie viabilitdu systme value de l'extrieur:
    ...
    if mode == "ynor":
    ...
    elif mode == "random": # Contrôleur alatoire (Monkey)
        ...
        # On retourne la viabilitRÉELLE value par la nature
    ...
    elif mode == "ablation_no_D": # Optimise sans peur de l'incohrence (\alpha = 0)
...
def run_experiment_suite():
    ...
    for s in range(seeds):
        ...
        # Mêmgen stochastique par architecture pour comparaison iso
    ...
    for mode, data in results.items():
        ...
        print(f"[{nom}] -> Compute Moyen: {avg_step:4.1f} steps | ViabilitFinale (\u03bc): {avg_L:6.3f} | Variance: {var_L:5.4f}")
