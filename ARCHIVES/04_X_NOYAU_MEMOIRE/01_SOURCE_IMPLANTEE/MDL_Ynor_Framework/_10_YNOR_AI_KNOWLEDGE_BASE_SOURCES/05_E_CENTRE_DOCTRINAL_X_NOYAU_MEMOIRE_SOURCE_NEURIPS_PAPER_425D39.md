...
To empirically validéte Theorem 1, we conduct a rigorous ablation study. Disabling the cost penalty ($\beta = 0$) causes the agent to devolve into pathological overthinking ($C=15$, $\mu = -0.080$). Disabling the divergence penalty ($\alpha = 0$) triggers premature halting (hallucinations, mean 4.1 steps, $\mu = +0.545$). 
