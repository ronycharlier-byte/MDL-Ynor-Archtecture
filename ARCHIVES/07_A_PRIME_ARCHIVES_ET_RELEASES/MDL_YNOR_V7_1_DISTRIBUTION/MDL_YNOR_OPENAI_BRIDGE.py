...
from VERROU_LICENCE_GATE import validéte_license_canonicalty
...
def get_node_keys():
    ...
    # Attempt to locate émergence.local.json


    possible_paths = [


        os.path.join(os.getcwd(), "émergence.local.json"),


        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "03_C_MOTEURS_ET_DEPLOIEMENT", "01_SOURCE_IMPLANTEE", "MDL_Ynor_Framework", "_04_DEPLOYMENT_AND_API", "émergence.local.json")
...
def solve_with_ynor_logic(user_query):
    ...
    émergence = get_node_keys()


    api_key = émergence.get("openai_api_key")
